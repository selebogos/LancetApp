...
@Component({
  selector: 'app-customertypes',
  templateUrl: './customertypes.component.html',
  styleUrls: ['./customertypes.component.css']
})
export class CustomertypesComponent implements OnInit {
    ...
    this.dtOptions = {
      ...
      },
      {
        ...
        render: function (data: any, type: any, full: any) {
          return '<button type="button" class="btn btn-warning" view-customer-type-id="' + full.Id + '">View</button>';
        }
      }
    ]
    };
  ...
  ngAfterViewInit(): void {
    this.renderer.listenGlobal('document', 'click', (event) => {
      if (event.target.hasAttribute("view-customer-type-id")) {
        this.router.navigate(["/customertypedetails/" + event.target.getAttribute("view-customer-type-id")]);
      }
    });
  }
