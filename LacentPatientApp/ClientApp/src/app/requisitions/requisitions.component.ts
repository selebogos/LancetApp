...
export class RequisitionsComponent implements OnInit {
    ...
    this.dtOptions = {
      ...
      },
      {
        ...
        render: function (data: any, type: any, full: any) {
          debugger;
          return '<button type="button" class="btn btn-warning" view-patients-id="' + full.Id + '">View</button>';
        }
      }
    ]
    };
  ...
  ngAfterViewInit(): void {
    this.renderer.listenGlobal('document', 'click', (event) => {
      if (event.target.hasAttribute("view-patients-id")) {
        this.router.navigate(["/requisitiondetails/" + event.target.getAttribute("view-patients-id")]);
      }
    });
  }

  goToCustomerDetails(id:number) {
    this.router.navigate(['/requisitiondetails', id]);
  }
