    ...
    this.dtOptions = {
      ...
      },
      {
        ...
        render: function (data: any, type: any, full: any) {
          return '<button type="button" class="btn btn-warning" view-user-id="' + full.Id + '">View</button>';
        }
      }
    ]
    };
  ...
  ngAfterViewInit(): void {
    this.renderer.listenGlobal('document', 'click', (event) => {
      if (event.target.hasAttribute("view-user-id")) {
        this.router.navigate(["/userdetails/" + event.target.getAttribute("view-user-id")]);
      }
    });
  }
