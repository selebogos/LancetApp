...
@Component({
  selector: 'app-cashups',
  templateUrl: './cashups.component.html',
  styleUrls: ['./cashups.component.css']
})
export class CashupsComponent implements AfterViewInit, OnInit {
  ...
  ngAfterViewInit(): void {
    this.renderer.listenGlobal('document', 'click', (event) => {
      if (event.target.hasAttribute("view-pos-id")) {
        this.router.navigate(["/cashupdetails/" + event.target.getAttribute("view-pos-id")]);
      }
    });
  }

  goToCustomerDetails(id:number) {
    this.router.navigate(['/cashupdetails', id]);
  }
